...
class LatihanController extends Controller
{
    ...
    public function data()
    {
        ...
        $query = DB::table('checklist_data as a')
                ...
                ->select('a.*')       
                ->where('a.status', 'A')->where('b.id_menu', 80)
        ...
        return DataTables::of($query)
           ...
           ->addColumn('pelabuhan', function ($row) {
            ...
            ->addColumn('aksi', function ($row) {
                return view('ck_kapal.latihan.partials.actions', compact('row'))->render();
            })
            ->rawColumns(['aksi'])
            ->make(true);
    }
    ...
    public function pdf($uid) {
        ...
        $pdf = Pdf::loadView('data_crew.latihan.pdf', $data)
    ...
    {
        $result = ChecklistDataDetail::where('checklist_data_id', $id)->where('status', 'A')->get();
        if (!$result) {
    ...
    public function elemen($uid)
    {
        ...
        return view('ck_kapal.latihan.elemen', $data);
    }
