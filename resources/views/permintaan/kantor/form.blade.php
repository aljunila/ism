        ...
        $.ajax({
            ...
            success: function(response){
                    Swal.fire({
                        ...
                        window.location.href = "{{ url('/permintaan/kantor') }}";
                    });
            },
            error: function(xhr){
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    data-store-url="{{ route('kantor.store') }}" data-update-url="{{ isset($data) ? route('kantor.update', $data->id) : '' }}">
                    @csrf
                    <div class="row">
                        <div class="col-10">
                            <div class="mb-1 row">
                                ...
                                <div class="col-sm-3">
                                    <select name="id_cabang" id="id_cabang" class="form-control" {{ isset($data) ? 'disabled' : '' }}>
                                        <option value="">Pilih Cabang</option>
                                        @foreach($cabang as $kp)
                                            <option value="{{$kp->id}}" @selected (isset($data) && $kp->id==$data->id_cabang)>{{$kp->cabang}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="mb-1 row">
                                ...
                                <div class="col-sm-3">
                                    <select name="bagian" id="bagian" class="form-control" {{ isset($data) ? 'disabled' : '' }}>
                                        ...
                                        @foreach($divisi as $d)
                                            <option value="{{$d->id}}" @selected (isset($data) && $d->id==$data->bagian)>{{$d->nama}}</option>
                                        @endforeach 
                                    </select>
                                </div>
                            </div>
