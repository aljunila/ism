...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasTable('t_turun_barang')) {
            return;
        }

        Schema::table('t_turun_barang', function (Blueprint $table) {
            if (!Schema::hasColumn('t_turun_barang', 'id_penerima')) {
                $table->unsignedInteger('id_penerima')->nullable()->after('id_kapal');
                $table->index('id_penerima');
            }

            if (!Schema::hasColumn('t_turun_barang', 'otp_code')) {
                $table->string('otp_code', 10)->nullable()->after('id_penerima');
            }

            if (!Schema::hasColumn('t_turun_barang', 'otp_verified_at')) {
